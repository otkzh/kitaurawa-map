...
Beforeはイベント前に取得した固定スナップショット `data/before-osm.json` を参照する。初期表示ではBefore JSONだけを読み込み、Afterは利用者が取得ボタンを押したときだけOverpass APIから取得する。
...
- 発表中に必要な場合だけ、ボタン操作でAfterデータを取得する。
...
- 画面下部の「Afterを取得」ボタンを押したときだけ、`survey:date=2026-06-07` またはイベントnoteキーワードに一致するOSMデータをOverpass APIから取得する。
- BeforeとAfterで同じOSM要素がある場合は、After側を優先して表示する。
...
- 初期表示ではBefore JSONのみを読み込み、After取得ボタンを押すまでOverpass APIへアクセスしない。
...
- Leaflet CSS / JSの取得
- 背景地図タイルの取得
- After取得ボタン押下後のOverpass APIへの問い合わせ
- OSM詳細ページへのリンク表示
...
- After取得中はボタンを無効化し、読み込み中であることを表示する。
- After取得成功後は「Afterを再取得」に文言を変える。
- After取得失敗時は、Before表示を維持したまま利用者にエラーを表示し、再試行できる状態に戻す。
...
- After取得ボタンで当日の成果地点が表示される。
- AfterのOverpass API取得失敗時にBefore表示が壊れない。
- レイヤー切り替えが反映される。
- カテゴリフィルターが反映される。
- 年フィルターが反映される。
