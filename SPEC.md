...
Beforeはイベント前に取得した固定スナップショット `data/before-osm.json` を参照する。MapLibre版の初期表示では、保存済み本日変更スナップショット `data/changed-osm.json` も読み込む。Overpass APIへのアクセスは利用者が再取得ボタンを押したときだけ行う。
...
- MapLibre版では、必要な場合だけ当日変更地物をOverpass APIから再取得して強調表示を更新する。
...
保存済みスナップショット `data/changed-osm.json` は、MapLibre版の初期表示で参照する。現在の保存状態:
...
- MapLibre版では「本日変更を再取得」ボタンを押したときだけ、日本時間 `2026-06-07 00:00` 以降に変更された範囲内のタグ付きOSM要素をOverpass APIから取得する。
...
- 初期表示では保存済みBefore JSONと保存済み本日変更JSONを読み込み、本日変更再取得ボタンを押すまでMapLibre版はOverpass APIへアクセスしない。

### ラベル表示

- ズームレベルが一定以上の場合、名称がある地点にラベルを表示する。
- ラベルが重なりすぎないよう、簡易的な衝突判定を行う。
- MapLibre版では、本日変更地物のラベルにOSMユーザ名を追加表示するチェックボックスを用意する。
...
- MapLibre版の本日変更再取得ボタン押下後のOverpass APIへの問い合わせ
- OSM詳細ページへのリンク表示
...
- MapLibre版の初回表示で保存済み本日変更地物が強調表示される。
- MapLibre版の本日変更再取得ボタンで当日変更地物が更新される。
