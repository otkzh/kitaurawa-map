...
Beforeはイベント前に取得した固定スナップショット `data/before-osm.json` を参照する。MapLibre版とLeaflet版の初期表示では、保存済み本日変更スナップショット `data/changed-osm.json` も読み込む。JSON更新ボタンは一時停止中とし、画面からOverpass APIへの再取得はできない。
...
- ローカル環境でブラウザから起動し、初期表示では保存済みBeforeデータと保存済み本日変更データを確認する。
- 両方の版で、当日変更地物を保存済みJSONから読み込み強調表示する。
- OSMを知らない人向けに、数字・カテゴリ・当日の例を大きく見せる簡易版を用意する。
...
表示対象タグに一致する既存OSMデータをBeforeとして扱う。

### 本日変更

MapLibre版とLeaflet版では、日本時間 `2026-06-07 00:00` 以降に変更された範囲内のタグ付きOSM要素を本日変更として扱う。UTC条件は `2026-06-06T15:00:00Z` とする。取得した要素は黄色い外枠で強調し、Beforeと同じOSM要素IDの場合は既存表示に強調状態を付与する。

保存済みスナップショット `data/changed-osm.json` は、両方の版の初期表示で参照する。現在の保存状態:
...
- 初期表示では `data/before-osm.json` と `data/changed-osm.json` を自動で読み込む。
- 初期表示時にはOverpass APIへアクセスしない。
- 両方の版でAfter表示を行わず、本日変更を成果確認モードとして扱う。
- JSON更新ボタンは一時停止中とし、クリックできない状態にする。
...
- 初期表示では保存済みBefore JSONと保存済み本日変更JSONを読み込み、両方の版でOverpass APIへアクセスしない。

### ラベル表示

- ズームレベルが一定以上の場合、名称がある地点にラベルを表示する。
- ラベルが重なりすぎないよう、簡易的な衝突判定を行う。
- MapLibre版とLeaflet版では、本日変更地物のラベルにOSMユーザ名を追加表示するチェックボックスを用意する。
...
- `index.html`（MapLibre版）
- `leaflet.html`（Leaflet版）
- `simple.html`（簡易版）
- `src/main.js`
- `src/maplibre-main.js`
- `src/simple.js`
- `src/styles.css`
- `src/simple.css`
- `README.md`
...
メインのMapLibre版は `index.html`、補助のLeaflet版は `leaflet.html` とする。
OSMを知らない人向けの簡易版は `simple.html` とする。
...
- Leaflet CSS / JSの取得
- 背景地図タイルの取得
- JSON更新ボタン再有効化後のOverpass APIへの問い合わせ
- OSM詳細ページへのリンク表示
...
| 設定名 | 初期値 | 用途 |
| --- | --- | --- |
| `CHANGED_SINCE_UTC` | `2026-06-06T15:00:00Z` | 本日変更判定に使うUTC日時 |
...
- MapLibre版とLeaflet版の初回表示で保存済み本日変更地物が強調表示される。
- JSON更新ボタンが押せない状態になっている。
...
1. MapLibre版を`index.html`として動く状態にする。
